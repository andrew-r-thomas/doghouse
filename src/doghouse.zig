...
pub fn Yin(comptime size: usize, comptime thresh: f32, comptime lb: usize) type {
    ...
    return struct {
        pub fn detect_pitch(signal: [size]f32, sample_rate: usize) f32 {
            ...
            var diffs: [window_size - lb]f32 = undefined;
            for (0..(window_size - lb)) |lag| {
                diffs[lag] = diff_fn(lag + lb, signal);
            }
        ...
        fn diff_fn(lag: usize, signal: [size]f32) f32 {
            const x: @Vector(window_size - lb, f32) = signal[lb..window_size].*;
            // this is a little silly, i feel like the compiler could be smarter
            const x_lag: @Vector(window_size - lb, f32) = signal[lag..(lag + window_size)][lb..window_size].*;
        ...
        fn cmndf(lag: usize, diffs: [window_size - lb]f32) f32 {
            if (lag == 0) return 1;
...
test "simple test" {
    ...
    for (0..1024) |i| {
        ...
        const val: f32 = @sin(float_idx * sec_per_samp * std.math.pi * 2 * 440);
        note[i] = val;
    }
    const yin = Yin(1024, 0.01);
    const pitch = yin.detect_pitch(note, 44100);
    try std.testing.expect(std.math.approxEqAbs(f64, pitch, 440, 5));
}
